...
4. **running-gates** — execute the spec's Verification section literally, item by item, proof attached per item. Records **claimed-vs-verified delta** (the agent's completion claim vs. gate findings — perception-gap data for steward). Includes a **traceability section**: every change maps to a plan item maps to a requirement; orphan changes listed. Agent self-assessment is never a completion signal.
...
6. **instrumenting-sessions** — hybrid: session-start is a SessionStart hook (deterministic — cctx watch on, purpose tags); session-end self-autopsy is a skill (judgment — what shipped, cost, deviations, harvest candidates, open threads).
...
**UX decision still open:** `/gg` as a single front door that routes by context ("which boundary applies right now?") vs. eight separately-triggered skills. Decide before skill descriptions are written.
...
- Four required capabilities (inherited from the Eddy brief): live read of what changed; drift measured against stated intent; the human's judgment captured *before* the AI's framing; trends, not snapshots.
...
1. **Artifact formats** — *drafted 2026-06-09*: templates in `templates/`, worked examples (one coherent fictional work item) in `examples/specs/06-orchestrator/`. Conventions (also in `templates/README.md`): artifacts live in `specs/<NN>-<slug>/`; frontmatter is for machines (steward's trend dataset), body for people; brevity is a floor; every field must have a reader — harvest deletes fields nobody reads. Key rules baked in: questions carry a default-if-unanswered; the gate claim is recorded *before* gates run; escalation `scope`/`blocks` keys power the parking hook and session-end rule; drift `trigger` key audits whether tripwires fire. Remaining: triage verdict format (the work wedge) and mini-spec format.
2. **`/gg` front door vs. eight triggers** — decide before skill descriptions are written.
3. **Gate library wiring** — exact mechanics of `running-gates` wrapping jaqal gates and emitting the evidence report.
4. **Hook implementation pass** — the §6 tripwires as actual hook configs; verify counter/manifest mechanics in practice.
5. **superpowers classification table** (§9).
6. **steward practice template** — the own-read-then-compare markdown ritual, usable pre-software.
